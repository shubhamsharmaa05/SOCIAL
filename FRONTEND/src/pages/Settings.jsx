  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-[#A8A8A8] mt-1">Configure your workspace and AI preferences.</p>
      </div>

      <div className="flex flex-col md:flex-row gap-8">
        <div className="w-full md:w-64 space-y-2">
          {['General', 'Brand Voice', 'Autonomous Mode', 'Notifications', 'Billing'].map((tab, i) => (
            <button key={i} className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${i === 2 ? 'bg-primary/10 text-[#F50057] border border-primary/20' : 'text-[#A8A8A8] hover:text-white hover:bg-white/5'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 glass-panel p-6 rounded-xl">
          <div className="mb-6 pb-6 border-b border-[#363636] flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <Zap className="w-5 h-5 text-[#F50057]" /> Autonomous Mode
              </h2>
              <p className="text-sm text-[#A8A8A8] mt-1">Allow AI to schedule and publish automatically without approval.</p>
            </div>
            <div className="w-12 h-6 bg-[#121212] rounded-full relative cursor-pointer border border-[#363636]">
